...
- **Import order** is enforced by the `gci` formatter (see `.golangci.yml`): standard library, `github.com/stretchr/testify`, `tests/mocks`, third-party (`default`), `github.com/inference-gateway/*`, then this module. Fix locally with `golangci-lint fmt`. Every non-standard-library import must be explicitly named (`gin "github.com/gin-gonic/gin"`), enforced by `importas`; fix with `golangci-lint run --fix`.
