  ...
  "data": [
    {
      ...
      "revised_prompt": "A serene mountain lake at sunset with tall pine trees perfectly reflected in the calm water, digital art style, vibrant orange and purple sky."
    }
  ]
}
```

Set `"response_format": "b64_json"` to receive the image as a base64-encoded
string instead of a URL:

```bash
curl -X POST http://localhost:8080/v1/images/generations -d '{
  "model": "openai/dall-e-3",
  "prompt": "A cute corgi puppy playing in a field of flowers",
  "n": 1,
  "size": "1024x1024",
  "response_format": "b64_json"
}' | jq '.data[0].b64_json' -r | base64 -d > corgi.png
```
