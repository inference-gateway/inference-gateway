# Authentication with Amazon Cognito on Kubernetes

The [Keycloak example](../auth-keycloak/README.md) shows how the operator wires
OIDC into the gateway, and the [docker-compose Cognito example](../../docker-compose/auth-cognito/README.md)
explains the user pool, the machine app client and why the gateway checks
`client_id` instead of `aud`. This example reuses both: complete the compose
example's **Setup** section first. It writes the `.env` this Taskfile reads and
provides the token script it calls.

Cognito is a public HTTPS issuer, so no in-cluster identity provider, CA bundle
or DNS rewrite is needed.

## Prerequisites

- [Task](https://taskfile.dev/installation/), kubectl, helm, ctlptl
- `envsubst` (part of gettext, `brew install gettext` on macOS)
- `../../docker-compose/auth-cognito/.env` filled in by that example's setup
