...
| MCP_SERVERS                  | `""`          | Comma-separated list of MCP servers as alias=url or url. A missing alias is derived from the url host; aliases must match ^[a-z0-9_-]+$, be unique, and not be the reserved alias tools. Tools are exposed to models as mcp_<alias>_<tool name> |
